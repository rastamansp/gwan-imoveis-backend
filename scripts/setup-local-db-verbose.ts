 ...
 * 2. Marca todas as migrations como jÃ¡ executadas (evita re-aplicar sobre o schema existente)
...
async function setupLocalDb() {
  console.log('ðŸš€ Configurando banco de dados local...\n');
  ...
  console.log('ðŸ“¦ Fase 1: Criando schema a partir das entities...');
  ...
  console.log('ðŸ“‹ Fase 2: Marcando migrations como executadas...');
  ...
  for (const migration of MIGRATIONS_TO_MARK) {
    ...
    if (exists.length === 0) {
      ...
      console.log(`  âœ… Marcada: ${migration.name}`);
    } else {
      console.log(`  â­ï¸  JÃ¡ existe: ${migration.name}`);
    }
  }

  // Fase 3: criar usuÃ¡rio admin
  console.log('\nðŸ‘¤ Fase 3: Criando usuÃ¡rio admin padrÃ£o...');
  ...
  if (!existing) {
    ...
    console.log('  âœ… Admin criado: admin@gwan.cloud / pazdeDeus@2025');
  } else {
  ...
  console.log('\nðŸŽ‰ Banco de dados local configurado com sucesso!');
