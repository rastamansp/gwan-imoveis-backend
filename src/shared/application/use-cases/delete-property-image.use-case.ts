...
export class DeletePropertyImageUseCase {
  ...
  async execute(propertyId: string, imageId: string, corretorId: string): Promise<boolean> {
    const startTime = Date.now();

    this.logger.info('Deletando imagem de propriedade', {
      propertyId,
      imageId,
      corretorId,
    });
    ...
    try {
      ...
      if (corretor.role !== UserRole.CORRETOR && corretor.role !== UserRole.ADMIN) {
        throw new Error('Apenas corretores e administradores podem deletar imagens');
      }

      if (property.corretorId !== corretorId && corretor.role !== UserRole.ADMIN) {
        throw new Error('Apenas o dono da propriedade ou administrador podem deletar imagens');
      }
