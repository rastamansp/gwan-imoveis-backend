# 📚 Documentação - Gwan Events Backend

Bem-vindo à documentação completa do Gwan Events Backend, uma plataforma completa de eventos e venda de ingressos construída com NestJS e TypeScript.

## 📋 Índice

### 🏗️ Arquitetura
- [Visão Geral da Arquitetura](./architecture/overview.md)

### 🔧 Desenvolvimento
- [Guia de Configuração](./development/setup.md)
- [Documentação Automática](./development/auto-documentation.md)

### 🚀 APIs
- [Documentação da API](./api/overview.md)

### 🔌 MCP (Model Context Protocol)
- [Visão Geral do MCP](./mcp/overview.md)

### 🚀 Deploy e Produção
- [Deploy Automático](./deployment/deploy-automation.md)
- [Deploy com Docker](./deployment/docker.md)
- [Deploy com Portainer](./deployment/portainer.md)
- [Configuração de Ambiente](./deployment/environment.md)

### 📊 Diagramas
- [Arquitetura do Sistema](./diagrams/system-architecture.md)

## 🎯 Início Rápido

### Para Desenvolvedores
1. [Configuração do Ambiente](./development/setup.md)
2. [Documentação Automática](./development/auto-documentation.md)

### Para DevOps
1. [Configuração de Produção](./deployment/environment.md)
2. [Deploy Automático](./deployment/deploy-automation.md)

### Para Integração MCP
1. [Visão Geral do MCP](./mcp/overview.md)

## 🔗 Links Úteis

- [Swagger UI](http://localhost:3001/api) - Documentação interativa da API
- [MCP Server](./mcp/overview.md) - Servidor Model Context Protocol

## 📝 Contribuindo

Para contribuir com a documentação:

1. Use diagramas Mermaid quando apropriado
2. Mantenha a estrutura consistente
3. Atualize o índice quando necessário
4. Valide a documentação antes do commit

## 🆘 Suporte

- [GitHub Issues](https://github.com/seu-usuario/gwan-events-backend/issues)
